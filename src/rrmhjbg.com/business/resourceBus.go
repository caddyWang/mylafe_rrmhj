...
func RecordUserDownInfo(fileName string) (newSrcTipNum int) {
	...
	case RoleFaceType:
		...
		if len(keys) == 4 {
			...
			if !vaildDownKey(actionKey) || !vaildDownKey(clothingKey) {
				return
			}
...
func DownNewRole(roleName, uid string) (zipbyte []byte, zipName string) {
	...
	zipName = getUniqID() + "_" + strconv.Itoa(RoleType)
...
func DownSingleFace(faceName, uid string) (zipbyte []byte, zipName string) {
	...
	if isExist {
		...
		zipName = getUniqID() + "_" + strconv.Itoa(RoleFaceType)
...
func DownSingleAction(actionName, uid string) (zipbyte []byte, zipName string) {
	...
	if isExist {
		...
		zipName = getUniqID() + "_" + strconv.Itoa(RoleActionType)
		jsonRtn := tools.TransformJSON(NewDownRes{FileName: zipName, ImgStruct: confFile})
...
func DownSingleClothing(clothingName, uid string) (zipbyte []byte, zipName string) {
	...
	if isExist {
		...
		zipName = getUniqID() + "_" + strconv.Itoa(RoleClothingType)
		jsonRtn := tools.TransformJSON(NewDownRes{FileName: zipName, ImgStruct: confFile})
...
func DownSingleDialog(dialogName, uid string) (zipbyte []byte, zipName string) {
	...
	if isExist {
		...
		zipName = getUniqID() + "_" + strconv.Itoa(DialogType)
		jsonRtn := tools.TransformJSON(NewDownRes{FileName: zipName, ImgStruct: []DownRes{cf}})
...
func DownSingleScene(sceneName, uid string) (zipbyte []byte, zipName string) {
	...
	if isExist {
		...
		zipName = getUniqID() + "_" + strconv.Itoa(SceneType)
		jsonRtn := tools.TransformJSON(NewDownRes{FileName: zipName, ImgStruct: []DownRes{cf}})
...
func getUniqID() (string){
    ...
    var uuid string = string(out)
    uuid = strings.Replace(uuid, "\n", "", 1)
    */
    
    var uuid string = strconv.FormatInt(time.Now().UnixNano(), 10)

    return uuid
}

func vaildDownKey(fileName string) (bool){
	if srcCache.Get(fileName) == nil {
		beego.Error("[rrmhjbg.com/business/RecordUserDownInfo(fileName=", fileName, ")]")
		return false
	}
